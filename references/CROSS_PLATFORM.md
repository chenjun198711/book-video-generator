...
TTS 引擎配置（可选）：
- **火山引擎 TTS**（默认）：设置环境变量 `VOLC_TTS_API_KEY`（[新版控制台](https://console.volcengine.com/speech/new) → API Key 管理获取，单个 Key）
...
`compose_video.py` 和 `generate_cover.py` 均内置字体自动检测函数，无需手动修改。
...
| 2026-07-17 | 2.1 | Ken Burns 缩放 + 淡入淡出转场 + 字体自动检测 |
| 2026-07-17 | 2.2 | 封面图生成（generate_cover.py）+ compose_video cover 字段支持 |
| 2026-07-17 | 2.3 | 逐句单行字幕：edge-tts WordBoundary 词级时间戳 + 按标点拆分 + 精确语音同步 + 字体增大至36pt |
| 2026-07-17 | 2.4 | TTS 双引擎：火山引擎 TTS 默认（SAMI HTTP API，enable_timestamp 词级时间戳）+ edge-tts 备选（自动检测凭证切换） |
| 2026-07-17 | 2.5 | 火山引擎改用 V1 API + X-Api-Key 认证（单个 Key），豆包语音合成 2.0 音色，基于音频时长估算词级时间戳 |
| 2026-07-17 | 2.6 | 顶部进度条（板块标题高亮+进度线）+ 关键词高亮（ASS 字幕格式，橙色标注关键词）+ Pillow 依赖 |
